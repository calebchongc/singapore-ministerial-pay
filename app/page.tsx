...
export default function Home() {
  ...
  const pileRowStep=(graphic==='money'?31.7:38)/Math.max(1,Math.ceil(Math.min(48,maximumIcons)/6)-1);
  ...
  return <main>
    ...
    <section id="sources" className="sources"><details><summary><span>Sources & how this works</span><ChevronDown size={18}/></summary><div className="source-body">
      ...
      <p>Bonuses model payouts, not the economic indicators used to decide them. Salary bands permit pay above and below the reference point. The value per icon stays fixed across roles and frameworks. Pile row spacing adapts to the selected role and framework so the maximum bonus fills the graphic area; spacing stays fixed while dragging. One money bundle represents S$100,000; each rice graphic represents 25,000 plates at the assumed S$4 per plate. Plate totals are rounded down to whole plates. One house represents ten months of median household income, not a property purchase. Partial icons are faded. Illustrations stop at 48 icons; numerical totals continue.</p>
