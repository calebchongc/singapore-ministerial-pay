...
import { calculate, parseAvc, type Role, type Framework, type Bonuses } from './salary';

type Scenario = {role:Role;framework:Framework;bonus:Bonuses;avc:number};
  ...
  useEffect(()=>{
    ...
    const tool={name:'configure_salary_scenario',description:'Set the visible Singapore salary calculator to a reference framework scenario and return its annual components. This is not personal salary disclosure.',
      ...
      execute(input:unknown){
        ...
        const scenario:Scenario={role:p.role,framework:p.framework,bonus:{performance:p.performance,national:p.national},avc:p.avc};
        flushSync(()=>apply(scenario));
        return {role:p.role,framework:p.framework,...calculate(scenario.role,scenario.framework,scenario.bonus,scenario.avc)};
      }};
